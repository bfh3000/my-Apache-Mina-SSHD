...
public class ServerDaemon2st {
    private static final Logger logger = LogManager.getLogger(ServerDaemon2st.class);
    ...
    private static class Daemon extends Thread {
        ...
        public Daemon() {
            this.client = new ClientDaemon2st();
            ...
            CustomShellFactory2st shellFactory = new CustomShellFactory2st(this.client);
                ...
                public boolean authenticate(String username, String password, ServerSession session)
                    ...
                    iosvc.setIoServiceEventListener(new IoServiceEventListener() {
                        ...
                        public void connectionAccepted(IoAcceptor acceptor, SocketAddress local, SocketAddress remote, SocketAddress service) throws IOException {
                            logger.info(String.format(" ---------------- %d. connectionAccepted", ++(ServerDaemon2st.step)));
                        ...
                        public void abortAcceptedConnection(IoAcceptor acceptor, SocketAddress local,
                            ...
                            logger.info(String.format(" ---------------- %d. abortAcceptedConnection", ++(ServerDaemon2st.step)));
                        ...
                        public void abortEstablishedConnection(IoConnector connector, SocketAddress local,
                                ...
                                throws IOException {
                            logger.info(String.format(" ---------------- %d. abortEstablishedConnection", ++(ServerDaemon2st.step)));
                        ...
                        public void connectionEstablished(IoConnector connector, SocketAddress local,
                            ...
                            logger.info(String.format(" ---------------- %d. connectionEstablished", ++(ServerDaemon2st.step)));
                        }
                    });
                    ...
                    session.setReservedSessionMessagesHandler(new ReservedSessionMessagesHandler() {
                        ...
                        public void handleDebugMessage(Session session, Buffer buffer) throws Exception {
                            logger.info(String.format(" ---------------- %d. handleDebugMessage", ++(ServerDaemon2st.step)));
                        ...
                        public void handleIgnoreMessage(Session session, Buffer buffer) throws Exception {
                            logger.info(String.format(" ---------------- %d. handleIgnoreMessage", ++(ServerDaemon2st.step)));
                        ...
                        public boolean handleUnimplementedMessage(Session session, int cmd, Buffer buffer)
                                throws Exception {
                            logger.info(String.format(" ---------------- %d. handleUnimplementedMessage", ++(ServerDaemon2st.step)));
                        ...
                        public IoWriteFuture sendIdentification(Session session, String version,
                            ...
                            logger.info(String.format(" ---------------- %d. sendIdentification", ++(ServerDaemon2st.step)));
                        ...
                        public IoWriteFuture sendKexInitRequest(Session session,
                            ...
                            logger.info(String.format(" ---------------- %d. sendKexInitRequest", ++(ServerDaemon2st.step)));
                        ...
                        public boolean sendReservedHeartbeat(ConnectionService service) throws Exception {
                            logger.info(String.format(" ---------------- %d. sendReservedHeartbeat", ++(ServerDaemon2st.step)));
            ...
            this.sshd.addSessionListener(new SessionListener() {
                ...
                public void sessionEstablished(Session session) {
                    logger.info(String.format(" ---------------- %d. sessionEstablished", ++(ServerDaemon2st.step)));
                    SessionListener.super.sessionEstablished(session);
                }

                @Override
                public void sessionCreated(Session session) {
                    logger.info(String.format(" ---------------- %d. sessionCreated", ++(ServerDaemon2st.step)));
                    SessionListener.super.sessionCreated(session);
                }

                @Override
                public void sessionEvent(Session session, Event event) {
                    logger.info(String.format(" ---------------- %d. sessionEvent", ++(ServerDaemon2st.step)));
                ...
                public void sessionException(Session session, Throwable t) {
                    logger.info(String.format(" ---------------- %d. sessionException", ++(ServerDaemon2st.step)));
                ...
                public void sessionPeerIdentificationSend(Session session, String version, List<String> extraLines) {
                    logger.info(String.format(" ---------------- %d. sessionPeerIdentificationSend", ++(ServerDaemon2st.step)));
                ...
                public void sessionPeerIdentificationLine(Session session, String line, List<String> extraLines) {
                    logger.info(String.format(" ---------------- %d. sessionPeerIdentificationLine", ++(ServerDaemon2st.step)));
                ...
                public void sessionPeerIdentificationReceived(Session session, String version, List<String> extraLines) {
                    logger.info(String.format(" ---------------- %d. sessionPeerIdentificationReceived", ++(ServerDaemon2st.step)));
                ...
                public void sessionDisconnect(Session session, int reason, String msg, String language, boolean initiator) {
                    logger.info(String.format(" ---------------- %d. sessionDisconnect", ++(ServerDaemon2st.step)));
                    SessionListener.super.sessionDisconnect(session, reason, msg, language, initiator);
                }

                @Override
                public void sessionClosed(Session session) {
                    logger.info(String.format(" ---------------- %d. sessionClosed", ++(ServerDaemon2st.step)));
                ...
                public void sessionNegotiationStart(Session session, Map<KexProposalOption, String> clientProposal,
                    ...
                    logger.info(String.format(" ---------------- %d. sessionNegotiationStart", ++(ServerDaemon2st.step)));
                ...
                public void sessionNegotiationEnd(Session session, Map<KexProposalOption, String> clientProposal,
                                                  ...
                                                  Throwable reason) {
                    logger.info(String.format(" ---------------- %d. sessionNegotiationEnd", ++(ServerDaemon2st.step)));
                ...
                public void sessionNegotiationOptionsCreated(Session session, Map<KexProposalOption, String> proposal) {
                    logger.info(String.format(" ---------------- %d. sessionNegotiationOptionsCreated", ++(ServerDaemon2st.step)));
